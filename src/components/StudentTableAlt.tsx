// StudentTableAlt.tsx (From front end code)
...
interface TableBodyRowsProps {
  ...
  // NEW PROPS - passed down from StudentTableAlt
  myId: number;
  myFirstName: string;
...
const UpdateConfirmationModal = (props: {
  ...
  // These states are now managed by the parent (StudentTableAlt) and passed as props
...
const StudentTableAlt = (props: { thePages: RowPage[] }) => {
  ...
  // --- Moved state variables and submission logic from TableBodyRows to StudentTableAlt ---
  const [myFirstName, setMyFirstName] = useState("");
  const [myLastName, setMyLastName] = useState("");
  const [myEmail, setMyEmail] = useState("");
  const [myMajor, setMyMajor] = useState("");
  ...
  // const apiURL = import.meta.env.VITE_API_GO_URL; // Declare apiURL here
  ...
  const handleNewStudentSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const BASE_URL =
        import.meta.env.VITE_API_GO_URL ||
        import.meta.env.VITE_API_GO_URL_LOCALHOST;

      const sessionToken = "sampleTokenIguess"; // Use a real session token here

      // Form the data object from the state variables here in StudentTableAlt
      const formData = {
        id: newMyID,
...
export default StudentTableAlt;
